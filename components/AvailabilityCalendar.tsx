...
const AvailabilityCalendar: React.FC<AvailabilityCalendarProps> = ({ roomId, bookings, onDateSelect }) => {
  ...
  return (
    <div className="p-6 bg-white rounded-xl shadow-lg border border-gray-100 animate-fade-in-up">
      {/* Header */}
      <div className="flex justify-between items-center mb-8 px-2">
        <div className="flex items-center text-secondary">
          <CalendarIcon className="mr-3 text-primary" size={22}/>
          <h4 className="font-bold text-xl font-serif tracking-wide">{format(currentMonth, 'MMMM yyyy')}</h4>
        </div>
        <div className="flex space-x-3">
          <button onClick={prevMonth} className="p-2 bg-gray-50 hover:bg-gray-200 rounded-full transition-colors duration-200 text-gray-600 shadow-sm border border-gray-100"><ChevronLeft size={18}/></button>
          <button onClick={nextMonth} className="p-2 bg-gray-50 hover:bg-gray-200 rounded-full transition-colors duration-200 text-gray-600 shadow-sm border border-gray-100"><ChevronRight size={18}/></button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1 text-center mb-4">
        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
          <div key={d} className="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-widest">{d}</div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-y-2">
        {getDaysInMonth().map((date, idx) => {
          ...
          let bgClass = 'hover:bg-green-50 cursor-pointer text-gray-700';
          
          if (past) {
            bgClass = 'text-gray-300 cursor-not-allowed hover:bg-transparent';
          } else if (booked) {
            bgClass = 'bg-red-50/50 text-red-300 cursor-not-allowed';
      ...
      {/* Legend */}
      <div className="mt-8 pt-4 border-t border-gray-100 flex justify-center items-center text-xs text-gray-500 space-x-6">
